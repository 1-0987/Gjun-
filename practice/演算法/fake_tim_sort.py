...
from lession_merge_sort import merge, merge_sort
import time
import numpy as np
...
if __name__ == '__main__':
    ...
    print("Insertion sort time:", round(time.perf_counter() - start, 5))

    start = time.perf_counter()
    merge_sort(numbers)
    print("Merge sort time:", round(time.perf_counter() - start, 5))

    start = time.perf_counter()
    fake_timsort(numbers)
    print("fake timsort time:", round(time.perf_counter() - start, 5))
